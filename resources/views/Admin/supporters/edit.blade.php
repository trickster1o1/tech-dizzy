        ...
        <div class="col-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    ...
                    <form class="forms-sample" action="{{ route('supporter.update', $supporter->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="tab-content col-md-12" id="myTabContent">
                            <div class="tab-pane fade show active container" id="home" role="tabpanel"
                                ...
                                <input type="hidden" name="redirects_to" value="{{ URL::previous() }}">
                                    ...
                                    @include('inputs.text',$data=['title','Title*','Enter Title','text',$supporter->title])
                                    @include('inputs.text',$data=['website','Website','Enter Website','text',$supporter->website])
                                    @include('inputs.mediainput',$data=['logo','Logo (115x90px)','Upload Logo',$supporter->logo])
                                    @include('inputs.selects',[$data=['status','Status'], $option=['Active','Inactive'], $default=$supporter->status])
