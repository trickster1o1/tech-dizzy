        ...
        <div class="col-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    ...
                    <form class="forms-sample" action="{{ route('volunteer.store') }}" method="POST">
                        @csrf
                        <div class="tab-content col-md-12" id="myTabContent">
                            <div class="tab-pane fade show active container" id="home" role="tabpanel"
                                ...
                                <div class="row">
                                    ...
                                    @include('inputs.text',$data=['email','Email*','Enter Email Address','email'])
                                    @include('inputs.text',$data=['address','Address*','Enter Address','text'])
                                    @include('inputs.date',$data=['dob','Date Of Birth'])
                                    @include('inputs.text',$data=['occupation','Occupation','Enter Occupation','text'])
                                    @include('inputs.mediainput',$data=['image','Picture (370x412px)','Upload Your Image',''])
                                    @include('inputs.mediainput',$data=['attachment','Attachment','Link The File',''])
                                    @include('inputs.selects',[$data=['accepted','Accepted'], $option=['Yes','No'], $default=''])
